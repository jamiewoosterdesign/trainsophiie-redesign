...
export default function WizardFormContentService({ mode, step, formData, onChange, activeField, onSwitchMode }) {
    ...
    if (step === 1) {
        const isHeater = formData.serviceName && formData.serviceName.toLowerCase().includes('heater');
        const isHotWater = formData.serviceName && (formData.serviceName.toLowerCase().includes('hot water') || formData.serviceName.toLowerCase().includes('boiler'));
                ...
                <div>
                    ...
                    {showAutoFillBanner && (
                        <WizardAutoFillBanner
                            ...
                            description={
                                <>I found pricing and details for <strong>{isHotWater ? "Hot Water Systems" : "Heaters"}</strong> in your <strong>SOP_Manual.pdf</strong>. Want me to auto-fill this?</>
                            }
                            onAutoFill={() => {
                                ...
                                if (isHotWater) {
                                    ...
                                    onChange('description', "Complete diagnosis, repair, and installation of hot water systems. We specialize in gas, electric, and solar units.");
                                    onChange('price', "220.00");
                                    onChange('priceMode', 'fixed');
                                    onChange('durationValue', "90");
                                    ...
                                    onChange('questions', [
                                        {
                                            id: '1',
                                            text: "Where is the unit located?",
                                            options: [
                                                { id: 'opt1', text: 'Indoors' },
                                                { id: 'opt2', text: 'Outdoors' }
                                            ],
                                            isAutoGenerated: true
                                        },
                                        {
                                            id: '2',
                                            text: "What type of system is it?",
                                            options: [
                                                { id: 'opt1', text: 'Gas' },
                                                { id: 'opt2', text: 'Electric' },
                                                { id: 'opt3', text: 'Solar' }
                                            ],
                                            isAutoGenerated: true
                                        },
                                        { id: '3', text: "Is the unit currently leaking?", options: [], isAutoGenerated: true }
                                    ]);
                                    onChange('serviceOutcome', 'booking');
                                    onChange('serviceClosingScript', "I'll schedule a technician to come out and inspect your hot water system immediately.");

                                } else {
                                    // HEATER CONFIGURATION (Default/Fallthrough)
